...
export const WalletDetails: React.FC = () => {
  ...
  const { state, deleteTransaction, transferFunds, updateWallet, deleteWallet, addTransaction } = useFinance();
  ...
  const AddTransactionForm = () => (
      <form onSubmit={handleAddTransaction} className="space-y-6">
        <div className="grid grid-cols-2 gap-0 border border-white/10 rounded-sm overflow-hidden">
            ...
            className={`p-3 font-mono text-sm transition-colors ${addType === 'EXPENSE' ? 'bg-neon-pink text-black font-bold' : 'bg-transparent text-gray-400 hover:text-white'}`}
            >
            EXPENSE
            </button>
            <button
            ...
            className={`p-3 font-mono text-sm transition-colors ${addType === 'INCOME' ? 'bg-neon-green text-black font-bold' : 'bg-transparent text-gray-400 hover:text-white'}`}
            >
            INCOME
            </button>
        </div>
    ...
    <div className="p-8 pb-20 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        <div className="md:col-span-2 space-y-2">
            <div className="flex items-center justify-between md:justify-start md:gap-4">
                <h1 className="text-4xl md:text-5xl font-sans font-bold text-white tracking-tighter uppercase truncate">
                    {wallet.name}
                </h1>
                <button 
            ...
            <div className="flex items-center gap-2">
                <span className="px-2 py-0.5 border border-white/20 rounded text-xs text-gray-400 font-mono">
                    {wallet.type}
                </span>
                <span className="px-2 py-0.5 border border-white/20 rounded text-xs text-gray-400 font-mono" style={{ color: wallet.color }}>
                    {wallet.baseCurrency}
                </span>
            </div>
        </div>
      ...
      <div className="mb-12">
          <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-sans text-white flex items-center gap-2">
              ...
              <div className="flex bg-black border border-white/10 rounded-sm p-1">
                  <button 
                      ...
                      className={`text-xs font-mono uppercase px-3 py-1.5 rounded-sm transition-all ${chartTab === 'EXPENSE' ? 'bg-neon-pink text-black font-bold' : 'text-gray-500 hover:text-white'}`}
                  >
                      Expense
                  </button>
                  <button 
                      onClick={() => setChartTab('INCOME')}
                      className={`text-xs font-mono uppercase px-3 py-1.5 rounded-sm transition-all ${chartTab === 'INCOME' ? 'bg-neon-green text-black font-bold' : 'text-gray-500 hover:text-white'}`}
                  >
                      Income
                  </button>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-64">
               <div className="md:col-span-1 bg-surface border border-white/5 p-6 relative flex flex-col justify-center">
               ...
               <div className="md:col-span-2 bg-surface border border-white/5 p-6 overflow-y-auto custom-scrollbar">
                   <div className="flex justify-between text-xs font-mono text-gray-500 uppercase mb-4 pb-2 border-b border-white/5">
                       <span>Category</span>
                       <span>Amount & %</span>
                   </div>
                   <div className="space-y-3">
                           ...
                           return (
                            <div key={entry.name} className="flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-white font-sans text-sm group-hover:text-neon-cyan transition-colors">{entry.name}</span>
                                </div>
                                <div className="font-mono text-sm text-gray-300 text-right">
                                    <div>{entry.value.toFixed(2)} <span className="text-xs text-gray-600">{wallet.baseCurrency}</span></div>
                       ...
                       {currentChartData.length === 0 && (
                           <div className="text-gray-600 font-mono text-sm italic">No transactions recorded yet.</div>
                       )}
                   </div>
               </div>
          </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between border-b border-white/5 pb-4">
            <h3 className="text-lg font-sans text-white">Ledger</h3>
        ...
        <div className="space-y-2">
            {transactions.map((tx, i) => (
                <motion.div
                    ...
                    className="flex items-center justify-between p-4 bg-surface/50 border border-transparent hover:border-white/10 hover:bg-surfaceHighlight transition-all cursor-pointer group rounded-sm"
                >
                    <div className="flex items-center gap-4">
                        ...
                        <div className="overflow-hidden">
                            <div className="text-white font-sans text-sm truncate max-w-[200px] md:max-w-md">{tx.description}</div>
                            <div className="text-gray-500 text-xs font-mono flex gap-2">
                                <span>{tx.date}</span>
            ...
            {transactions.length === 0 && (
                <div className="py-12 text-center text-gray-600 font-mono">No transactions found.</div>
            )}
        </div>
      </div>
      ...
      <AnimatePresence>
        {isTransferring && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                >
                    <button onClick={() => setIsTransferring(false)} className="absolute top-4 right-4 text-gray-500 hover:text-white">
                        <X size={24} />
                    </button>
                    
                    <h2 className="text-2xl font-sans text-white mb-6">Transfer Funds</h2>
                    
                    <form onSubmit={handleTransfer} className="space-y-6">
                        <div className="bg-white/5 p-4 rounded border border-white/5">
                            <label className="block text-xs font-mono text-gray-400 mb-1 uppercase">From</label>
                            <div className="text-white font-bold">{wallet.name}</div>
                            <div className="text-xs text-gray-500">Balance: {wallet.balance.toFixed(2)} {wallet.baseCurrency}</div>
                        </div>

                        <div className="flex justify-center -my-3 relative z-10">
                            <div className="bg-black border border-white/10 p-2 rounded-full text-neon-green">
                                <ArrowRightLeft size={16} className="rotate-90" />
                            </div>
                        </div>

                        <div>
                             <label className="block text-xs font-mono text-gray-400 mb-2 uppercase">To Wallet</label>
                             <select 
                        ...
                        <div>
                            <label className="block text-xs font-mono text-gray-400 mb-2 uppercase">Amount ({wallet.baseCurrency})</label>
                            <input 
                                type="number"
                                step="any"
                                required
